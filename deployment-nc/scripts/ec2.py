...
class Ec2Inventory(object):
    ...
    def read_settings(self):
        ...
        defaults = {
            'ec2': {
                'ini_fallback':
                os.path.join(os.path.dirname(__file__), 'ec2.ini'),
                'ini_path':
                os.path.join(
        ...
        ec2_ini_path = os.environ.get('EC2_INI_PATH',
                                      defaults['ec2']['ini_path'])
        ...
        self.route53_excluded_zones = [
            a for a in config.get('ec2', 'route53_excluded_zones').split(',')
            if a
        ]
        ...
        self.include_rds_clusters = config.getboolean('ec2',
        ...
        self.all_elasticache_nodes = config.getboolean('ec2',
        ...
        if not (self.boto_profile or os.environ.get('AWS_ACCESS_KEY_ID')
                or os.environ.get('AWS_PROFILE')):

            aws_access_key_id = config.get('credentials', 'aws_access_key_id')
            aws_secret_access_key = config.get('credentials',
                                               'aws_secret_access_key')
            aws_security_token = config.get('credentials',
                                            'aws_security_token')
        ...
        self.cache_path_cache = os.path.join(cache_dir,
                                             "%s.cache" % cache_name)
        self.cache_path_index = os.path.join(cache_dir,
                                             "%s.index" % cache_name)
        ...
        if config.has_option(
                'ec2',
                'instance_filters') or 'EC2_INSTANCE_FILTERS' in os.environ:
            filters = os.getenv('EC2_INSTANCE_FILTERS',
                                config.get('ec2', 'instance_filters')
    ...
    def connect_to_aws(self, module, region):
        ...
        if self.iam_role:
            ...
            role = sts_conn.assume_role(self.iam_role,
                ...
                tags.extend(
                    conn.get_all_tags(
                        filters={
                            ...
                            'resource-id': instance_ids[i:i + max_filter_value]
                        }))
    ...
    def get_rds_instances_by_region(self, region):
        ...
        if not HAS_BOTO3:
            self.fail_with_error(
                "Working with RDS instances requires boto3 - please install boto3 and try again",
            ...
            if conn:
                ...
                while True:
                    ...
                    for index, instance in enumerate(instances):
                        ...
                        instance.arn = db_instances['DBInstances'][index][
                            'DBInstanceArn']
    ...
    def include_rds_clusters_by_region(self, region):
        if not HAS_BOTO3:
            self.fail_with_error(
                "Working with RDS clusters requires boto3 - please install boto3 and try again",
        ...
        for c in clusters:
            ...
            try:
                ...
                tags = client.list_tags_for_resource(
                    ResourceName='arn:aws:rds:' + region + ':' + account_id +
                    ':cluster:' + c['DBClusterIdentifier'])
                c['Tags'] = tags['TagList']
        ...
        try:
            ...
            if conn:
                ...
                while _marker:
                    ...
                    _marker = response['DescribeCacheClustersResponse'][
    ...
    def add_instance(self, instance, region):
        ...
        if self.destination_format and self.destination_format_tags:
            ...
            if dest is None:
                dest = getattr(instance,
                               'tags').get(self.vpc_destination_variable, None)
        else:
            dest = getattr(instance, self.destination_variable, None)
            if dest is None:
                dest = getattr(instance, 'tags').get(self.destination_variable,
                                                     None)
        ...
        elif self.route53_enabled and self.route53_hostnames and hostname.endswith(
                self.route53_hostnames):
                ...
                for v in values:
                    ...
                    if self.nested_groups:
                        self.push_group(self.inventory, 'tags',
                                        self.to_safe("tag_" + k))
                        if v:
                            self.push_group(self.inventory,
                                            self.to_safe("tag_" + k), key)
        ...
        self.inventory["_meta"]["hostvars"][
            hostname] = self.get_host_info_dict_from_instance(instance)
    ...
    def add_rds_instance(self, instance, region):
        ...
        if self.group_by_security_group:
            try:
                if instance.security_group:
                    key = self.to_safe(
                        "security_group_" + instance.security_group.name)
                    self.push(self.inventory, key, hostname)
                    if self.nested_groups:
                        self.push_group(self.inventory, 'security_groups', key)
        ...
        if self.group_by_rds_engine:
            self.push(self.inventory, self.to_safe("rds_" + instance.engine),
                      hostname)
            if self.nested_groups:
        ...
        if self.group_by_rds_parameter_group:
            ...
            if self.nested_groups:
                self.push_group(self.inventory, 'rds_parameter_groups',
                                self.to_safe("rds_parameter_group_" +
                                             instance.parameter_group.name))
    ...
    def add_elasticache_cluster(self, cluster, region):
        ...
        if self.group_by_security_group and not is_redis:
            ...
            if 'SecurityGroups' in cluster and cluster['SecurityGroups'] is not None:
                for security_group in cluster['SecurityGroups']:
                    key = self.to_safe(
                        "security_group_" + security_group['SecurityGroupId'])
        ...
        if self.group_by_elasticache_engine and not is_redis:
            self.push(self.inventory,
                      self.to_safe("elasticache_" + cluster['Engine']), dest)
            if self.nested_groups:
        ...
        if self.group_by_elasticache_parameter_group:
            ...
            if self.nested_groups:
                self.push_group(self.inventory, 'elasticache_parameter_groups',
                                self.to_safe(cluster['CacheParameterGroup'][
                                    'CacheParameterGroupName']))

        # Inventory: Group by replication group
        if self.group_by_elasticache_replication_group and 'ReplicationGroupId' in cluster and cluster['ReplicationGroupId']:
            self.push(self.inventory,
                      self.to_safe("elasticache_replication_group_" +
                                   cluster['ReplicationGroupId']), dest)
            if self.nested_groups:
                self.push_group(self.inventory,
    ...
    def add_elasticache_node(self, node, cluster, region):
        ...
        if self.group_by_elasticache_engine:
            ...
            if self.nested_groups:
                self.push_group(
                    self.inventory, 'elasticache_engines',
                    self.to_safe("elasticache_" + cluster['Engine']))
        ...
        if self.group_by_elasticache_cluster:
            self.push(self.inventory,
                      self.to_safe(
                          "elasticache_cluster_" + cluster['CacheClusterId']),
                      dest)
    ...
    def get_instance_route53_names(self, instance):
        ...
        instance_attributes = [
            'public_dns_name', 'private_dns_name', 'ip_address',
            'private_ip_address'
        ]
    ...
    def get_host_info_dict_from_instance(self, instance):
        ...
        for key in vars(instance):
            ...
            if key == 'ec2__state':
                ...
                instance_vars[
                    'ec2_previous_state'] = instance.previous_state or ''
                instance_vars[
                    'ec2_previous_state_code'] = instance.previous_state_code
    ...
    def get_host_info_dict_from_describe_dict(self, describe_dict):
        ...
        for key in describe_dict:
            ...
            if key == 'ec2_configuration_endpoint' and value:
                host_info['ec2_configuration_endpoint_address'] = value[
                    'Address']
            ...
            if key == 'ec2_node_groups' and value:
                host_info['ec2_endpoint_address'] = value[0][
                    'PrimaryEndpoint']['Address']
                host_info['ec2_endpoint_port'] = value[0]['PrimaryEndpoint'][
                    'Port']
                replica_count = 0
                for node in value[0]['NodeGroupMembers']:
                    if node['CurrentRole'] == 'primary':
                        host_info['ec2_primary_cluster_address'] = node[
                            'ReadEndpoint']['Address']
                        host_info['ec2_primary_cluster_port'] = node[
                            'ReadEndpoint']['Port']
                        host_info['ec2_primary_cluster_id'] = node[
                            'CacheClusterId']
                    elif node['CurrentRole'] == 'replica':
                        host_info['ec2_replica_cluster_address_' + str(
                            replica_count)] = node['ReadEndpoint']['Address']
                        host_info['ec2_replica_cluster_port_' + str(
                            replica_count)] = node['ReadEndpoint']['Port']
            ...
            # Target: All Cache Clusters
                ...
                host_info['ec2_cache_parameter_group_name'] = value[
                    'CacheParameterGroupName']
                host_info['ec2_cache_parameter_apply_status'] = value[
